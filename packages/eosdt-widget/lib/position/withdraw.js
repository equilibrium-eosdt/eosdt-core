...
function WithdrawEOS(deps) {
    var _this = this;
    var account = deps.account, contract = deps.contract, maxToWithdrawFunc = deps.maxToWithdrawFunc;
    return {
        state: {},
            ...
            return __generator(this, function (_a) {
                w.update({
                    form: form_1.default({
                        ...
                        fields: ["amount"],
                        notification: {
                            title: "Excess collateral withdrawn",
                            message: "Successfully executed",
                        },
                        validate: {
                            amount: function (value) {
                                var error;
                                var maxToWithdraw = maxToWithdrawFunc ? maxToWithdrawFunc() : 0;
                                if (Number(value) > Number(maxToWithdraw)) {
                                    var max = maxToWithdraw
                                        ? Number(maxToWithdraw).toFixed(4)
                                        : 0;
                                    error = globals_1.t(templateObject_1 || (templateObject_1 = __makeTemplateObject(["EOS amount withdrawn should not be more than ", ""], ["EOS amount withdrawn should not be more than ", ""])), max);
                                }
                                else if (Number(value) > 100000000) {
                                    error = globals_1.t(templateObject_2 || (templateObject_2 = __makeTemplateObject(["Withdraw amount cannot exceed 100000000."], ["Withdraw amount cannot exceed 100000000."])));
                                }
                            ...
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    ...
                                    case 1:
                                        ...
                                        return [4, contract.deleteCollateral(account.name, amount, positions[0].position_id)];
                                    case 2:
                                        _a.sent();
        ...
        render: function (state, r) { return r(templateObject_6 || (templateObject_6 = __makeTemplateObject(["", ""], ["", ""])), state.form); },
    };
}
exports.default = WithdrawEOS;
